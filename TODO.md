...
- [ ] Add recursive directory transfers (upload/download folders in SFTP pane mode).
- [ ] Add host-key verification with `known_hosts` instead of insecure callback.
- [ ] Add explicit auth fields in profile form (auth type, key path, vault refs, password source).

## Future / Nice to Have
- [ ] Add support for importing `~/.ssh/config`.
